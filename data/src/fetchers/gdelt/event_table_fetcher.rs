...
/// Iterator for EventTable records from CSV files
pub struct EventTableIterator {
    csv_reader: csv::Reader<File>,
}

impl EventTableIterator {
    pub fn new(file_path: PathBuf) -> Result<Self> {
        let file = File::open(&file_path)
            .with_context(|| format!("Failed to open event table file: {:?}", file_path))?;
...
impl Iterator for EventTableIterator {
    type Item = EventTable;

    fn next(&mut self) -> Option<Self::Item> {
        let mut record = csv::StringRecord::new();
        match self.csv_reader.read_record(&mut record) {
            Ok(true) => match EventTable::try_from(record) {
                Ok(event) => Some(event),
                Err(e) => {
                    log::warn!("Failed to parse event record: {}", e);
...
/// Fetcher for EventTable data
pub struct EventTableFetcher {
...
impl EventTableFetcher {
    ...
    /// Fetch latest events asynchronously
    pub async fn fetch_latest_async(&mut self) -> Result<EventTableIterator> {
        let file_path = if self.is_translation {
            self.gdelt_fetcher.fetch_latest_export_translation().await?
        } else {
            self.gdelt_fetcher.fetch_latest_export().await?
        };

        self.last_fetch_time = Some(Utc::now());
        EventTableIterator::new(file_path)
    }

    /// Fetch events by date asynchronously
    pub async fn fetch_date_async(
        &mut self,
        date: DateTime<impl TimeZone>,
    ) -> Result<EventTableIterator> {
        let naive_date = date.naive_utc();
        let file_path = if self.is_translation {
            self.gdelt_fetcher
                .fetch_export_translation_by_date(naive_date)
                .await?
        } else {
            self.gdelt_fetcher.fetch_export_by_date(naive_date).await?
        };
    ...
    pub fn count_latest_records(&self) -> Result<usize> {
        let files = self
            .gdelt_fetcher
            .get_local_files_by_table(TableType::Export)?;
        if let Some(latest_file) = files.first() {
            EventTableIterator::count_records(latest_file)
        } else {
            Ok(0)
        }
    }
...
impl DataFetcher<EventTable, PathBuf> for EventTableFetcher {
    type FetchBatch = EventTableIterator;
    ...
    fn fetch_latest(&self) -> Result<Self::FetchBatch> {
        ...
        let file_path = if self.is_translation {
            rt.block_on(async { self.gdelt_fetcher.fetch_latest_export_translation().await })?
        } else {
            rt.block_on(async { self.gdelt_fetcher.fetch_latest_export().await })?
        };
    ...
    fn fetch_date(&self, date: DateTime<impl TimeZone>) -> Result<Self::FetchBatch> {
        ...
        let file_path = if self.is_translation {
            ...
            rt.block_on(async { self.gdelt_fetcher.fetch_export_by_date(naive_date).await })?
        };
...
impl RawDataFetcher for EventTableFetcher {
...
mod tests {
    ...
    fn create_test_fetcher() -> Result<(EventTableFetcher, TempDir, TempDir)> {
        let output_dir = TempDir::new()?;
        let temp_dir = TempDir::new()?;
        let fetcher = EventTableFetcher::new_v2(output_dir.path(), temp_dir.path())?;
        Ok((fetcher, output_dir, temp_dir))
    }

    #[test]
    fn test_event_table_fetcher_creation() -> Result<()> {
        init_test_logging();
        log::info!("Starting test_event_table_fetcher_creation");

        log::debug!("Creating test fetcher with temporary directories");
        let (_fetcher, _output_dir, _temp_dir) = create_test_fetcher()?;

        log::info!("Successfully created EventTableFetcher");
        log::debug!("Output dir: {:?}", _output_dir.path());
        log::debug!("Temp dir: {:?}", _temp_dir.path());

        log::info!("test_event_table_fetcher_creation completed successfully");
        Ok(())
    }

    #[test]
    fn test_event_table_fetcher_v3_creation() -> Result<()> {
        init_test_logging();
        log::info!("Starting test_event_table_fetcher_v3_creation");
        ...
        log::debug!("Creating EventTableFetcher with GDELT v3");
        let _fetcher = EventTableFetcher::new_v3(output_dir.path(), temp_dir.path())?;
        log::info!("Successfully created EventTableFetcher v3");

        log::info!("test_event_table_fetcher_v3_creation completed successfully");
        Ok(())
    }

    #[test]
    fn test_event_table_fetcher_with_translation() -> Result<()> {
        init_test_logging();
        log::info!("Starting test_event_table_fetcher_with_translation");
        ...
        log::info!("test_event_table_fetcher_with_translation completed successfully");
        Ok(())
    }
    ...
    fn test_url_link_generation() -> Result<()> {
        ...
        log::debug!("Validating URL contains 'export'");
    ...
    fn test_url_link_with_translation() -> Result<()> {
        ...
        log::debug!("Validating translation URL contains 'export'");
        assert!(url.as_str().contains("export"));
    ...
    fn test_custom_version_creation() -> Result<()> {
        ...
        log::debug!("Creating EventTableFetcher with custom version V3");
        let fetcher =
            EventTableFetcher::with_version(output_dir.path(), temp_dir.path(), GdeltVersion::V3)?;
