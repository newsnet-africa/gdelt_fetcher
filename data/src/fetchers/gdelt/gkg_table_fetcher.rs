...
/// Iterator for GKGTable records from CSV files
pub struct GKGTableIterator {
    csv_reader: csv::Reader<File>,
}

impl GKGTableIterator {
    pub fn new(file_path: PathBuf) -> Result<Self> {
        let file = File::open(&file_path)
            .with_context(|| format!("Failed to open GKG table file: {:?}", file_path))?;
...
impl Iterator for GKGTableIterator {
    type Item = GKGTable;

    fn next(&mut self) -> Option<Self::Item> {
        let mut record = csv::StringRecord::new();
        match self.csv_reader.read_record(&mut record) {
            Ok(true) => match GKGTable::try_from(record) {
                Ok(gkg) => Some(gkg),
                Err(e) => {
                    log::warn!("Failed to parse GKG record: {}", e);
...
/// Fetcher for GKGTable data
pub struct GKGTableFetcher {
...
impl GKGTableFetcher {
    ...
    /// Fetch latest GKG data asynchronously
    pub async fn fetch_latest_async(&mut self) -> Result<GKGTableIterator> {
        let file_path = if self.is_translation {
            self.gdelt_fetcher.fetch_latest_gkg_translation().await?
        } else {
            self.gdelt_fetcher.fetch_latest_gkg().await?
        };

        self.last_fetch_time = Some(Utc::now());
        GKGTableIterator::new(file_path)
    }

    /// Fetch GKG data by date asynchronously
    pub async fn fetch_date_async(
        &mut self,
        date: DateTime<impl TimeZone>,
    ) -> Result<GKGTableIterator> {
        let naive_date = date.naive_utc();
        let file_path = if self.is_translation {
            self.gdelt_fetcher
                .fetch_gkg_translation_by_date(naive_date)
                .await?
        } else {
            self.gdelt_fetcher.fetch_gkg_by_date(naive_date).await?
        };
    ...
    pub fn count_latest_records(&self) -> Result<usize> {
        let files = self
            .gdelt_fetcher
            .get_local_files_by_table(TableType::Gkg)?;
        if let Some(latest_file) = files.first() {
            GKGTableIterator::count_records(latest_file)
        } else {
            Ok(0)
        }
    }
...
impl DataFetcher<GKGTable, PathBuf> for GKGTableFetcher {
    type FetchBatch = GKGTableIterator;
    ...
    fn fetch_latest(&self) -> Result<Self::FetchBatch> {
        ...
        let file_path = if self.is_translation {
            rt.block_on(async { self.gdelt_fetcher.fetch_latest_gkg_translation().await })?
        } else {
            rt.block_on(async { self.gdelt_fetcher.fetch_latest_gkg().await })?
        };
    ...
    fn fetch_date(&self, date: DateTime<impl TimeZone>) -> Result<Self::FetchBatch> {
        ...
        let file_path = if self.is_translation {
            ...
            rt.block_on(async { self.gdelt_fetcher.fetch_gkg_by_date(naive_date).await })?
        };
...
impl RawDataFetcher for GKGTableFetcher {
...
mod tests {
    ...
    fn create_test_fetcher() -> Result<(GKGTableFetcher, TempDir, TempDir)> {
        let output_dir = TempDir::new()?;
        let temp_dir = TempDir::new()?;
        let fetcher = GKGTableFetcher::new_v2(output_dir.path(), temp_dir.path())?;
        Ok((fetcher, output_dir, temp_dir))
    }

    #[test]
    fn test_gkg_table_fetcher_creation() -> Result<()> {
        init_test_logging();
        log::info!("Starting test_gkg_table_fetcher_creation");

        log::debug!("Creating test fetcher with temporary directories");
        let (_fetcher, _output_dir, _temp_dir) = create_test_fetcher()?;

        log::info!("Successfully created GKGTableFetcher");
        log::debug!("Output dir: {:?}", _output_dir.path());
        log::debug!("Temp dir: {:?}", _temp_dir.path());

        log::info!("test_gkg_table_fetcher_creation completed successfully");
        Ok(())
    }

    #[test]
    fn test_gkg_table_fetcher_v3_creation() -> Result<()> {
        init_test_logging();
        log::info!("Starting test_gkg_table_fetcher_v3_creation");
        ...
        log::debug!("Creating GKGTableFetcher with GDELT v3");
        let _fetcher = GKGTableFetcher::new_v3(output_dir.path(), temp_dir.path())?;
        log::info!("Successfully created GKGTableFetcher v3");

        log::info!("test_gkg_table_fetcher_v3_creation completed successfully");
        Ok(())
    }

    #[test]
    fn test_gkg_table_fetcher_with_translation() -> Result<()> {
        init_test_logging();
        log::info!("Starting test_gkg_table_fetcher_with_translation");
        ...
        log::info!("test_gkg_table_fetcher_with_translation completed successfully");
        Ok(())
    }
    ...
    fn test_url_link_generation() -> Result<()> {
        ...
        log::debug!("Validating URL contains 'gkg'");
        assert!(url.as_str().contains("gkg"));
        log::debug!("Validating URL contains 'csv.zip' (GKG uses lowercase csv)");
        assert!(url.as_str().contains("csv.zip")); // Note: GKG uses lowercase csv
    ...
    fn test_url_link_with_translation() -> Result<()> {
        ...
        log::debug!("Validating translation URL contains 'gkg'");
        assert!(url.as_str().contains("gkg"));
    ...
    fn test_custom_version_creation() -> Result<()> {
        ...
        log::debug!("Creating GKGTableFetcher with custom version V3");
        let fetcher =
            GKGTableFetcher::with_version(output_dir.path(), temp_dir.path(), GdeltVersion::V3)?;
